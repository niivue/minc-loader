 ...
 */
export function printStructure(link, level) {
  var i
  var msg = ''
  ...
  Object.keys(link.attributes).forEach(function (name) {
    ...
    msg += link.name + ':' + name + ' ' + typeName(value) + '[' + value.length + '] '
...
export function findDataset(link, name) {
  let result
  if (link.name === name && link.type > 0) {
    result = link
  } else {
    link.children.find((child) => {
      result = findDataset(child, name)
...
export function findAttribute(link, name) {
  let result = link.attributes[name]
  if (result) return result

  link.children.find((child) => {
    result = findAttribute(child, name)
    return result !== undefined
  })
  return result
}

export function isRgbVolume(header, image) {
  var order = header.order
  return (
    image.array.constructor.name === 'Uint8Array' &&
    order.length > 0 &&
...
export function typeIsFloat(typ) {
  return typ >= type_enum.FLT && typ <= type_enum.DBL
}

export function scaleVoxels(image, image_min, image_max, valid_range, debug) {
  var new_abuf = new ArrayBuffer(image.array.length * Float32Array.BYTES_PER_ELEMENT)
...
export async function hdf5Reader(abuf, debug) {
  ...
  function hdf5GroupSymbolTable(lh, link) {
    ...
    var n_sym = getU16()
    if (debug) {
      console.log('hdf5GroupSymbolTable V' + ver + ' #' + n_sym + " '" + link.name + "'")
    }
    var i
    ...
    for (i = 0; i < 2 * superblk.gln_k; i += 1) {
      ...
      if (i < n_sym) {
        ...
        if (debug) {
          console.log(
            '    ' + i + ' O ' + link_name_offset + ' A ' + ohdr_address + ' T ' + cache_type + " '" + child.name + "'"
          )
        }
  ...
  function hdf5MsgLinkInfo(link) {
    ...
    if (debug) {
      console.log('hdf5MsgLinkInfo V' + ver + ' F' + flags + ' FH ' + fh_address + ' BT ' + bt_address)
    }
    var spp = tell()
  ...
  function hdf5MsgDatatype(sz) {
    ...
    switch (cls) {
      ...
      case 1 /* Float: uses bits 0,6 for byte order */:
        ...
        if (debug) {
          msg +=
            bit_offs + ' ' + bit_prec + ' ' + exp_loc + ' ' + exp_sz + ' ' + mnt_loc + ' ' + mnt_sz + ' ' + exp_bias
        }
  ...
  function hdf5MsgLink(link) {
    ...
    if (debug) {
      console.log('hdf5MsgLink V' + ver + ' F' + flags + ' T' + ltype + ' NM ' + child.name + ' OF ' + child.hdr_offset)
    }
    link.children.push(child)
  }
  ...
  function hdf5MsgAttrInfo(link) {
    ...
    if (debug) {
      console.log('hdf5MsgAttrInfo V' + ver + ' F' + flags + ' HP ' + fh_addr + ' AN ' + bt_addr)
    }
  ...
  function loadData(link) {
    if (link.chunk_size !== 0 && link.data_offset > 0 && link.data_offset < superblk.eof_addr) {
      ...
      if (link.data_offset > 0 && link.data_offset < superblk.eof_addr) {
        ...
        link.array = getArray(link.type, link.data_length, link.data_offset)
      } else {
        if (debug) {
  ...
  function hdf5V1ObjectHeader(link) {
    ...
    if (debug) {
      console.log(
        'hdf5V1ObjectHeader V' + oh.oh_ver + ' #M ' + oh.oh_n_msgs + ' RC ' + oh.oh_ref_cnt + ' HS ' + oh.oh_hdr_sz
      )
    }
 ...
 */
export function rgbVoxels(image) {
  var im = image.array
  var n = im.length
